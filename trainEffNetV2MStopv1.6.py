...
# ================== 하이퍼파라미터 ==================
IMG_SIZE = (224, 224)
BATCH_SIZE = 8       # 안정적 메모리 관리 (2보다 효율적)
EPOCHS = 30
SEED = 42
...
train_ds = train_ds.prefetch(tf.data.AUTOTUNE)   # cache(), repeat() 제거
...
# 안정형 스텝 제한
steps_per_epoch = min(5000, math.ceil(train_count / BATCH_SIZE))
validation_steps = min(1000, math.ceil(val_count / BATCH_SIZE))
...
base_model.trainable = False  # 1차 학습 시 고정
...
# ================== 테스트 평가 ==================
print("\n✅ 테스트셋 평가 중...")
test_steps = min(2000, math.ceil(test_count / BATCH_SIZE))
