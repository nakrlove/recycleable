...
# 0-1. 로그 파일 설정
# ============================================================
log_filename = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_training.log"

def clean_ansi(text):
    ansi_escape = re.compile(r'(?:\x1B[@-_][0-?]*[ -/]*[@-~])')
    return ansi_escape.sub('', text).replace('\b', '')
...
sys.stderr = sys.stdout  # 오류 로그도 파일에 기록
...
DATASET_PATH = 'trash_dataset_path/train'
...
def build_efficientnet_model(input_shape, num_classes):
    base_model = EfficientNetV2M(input_shape=input_shape, include_top=False, weights='imagenet')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if len(self.acc_history) >= self.patience:
            ...
            if acc_stable and loss_stable:
                print(f"\n {self.patience}번 연속 val_accuracy와 val_loss가 안정적이므로 학습 중단")
