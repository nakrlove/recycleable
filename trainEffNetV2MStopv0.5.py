...
train_dataset = tf.keras.utils.image_dataset_from_directory(
    ...
    validation_split=0.2,  # train: 80%, val+test: 20% 9/4일오전 13:30분
    subset='training',
    seed=SEED
)
...
val_test_dataset = tf.keras.utils.image_dataset_from_directory(
    DATASET_PATH,
    image_size=IMAGE_SIZE,
    batch_size=BATCH_SIZE,#9/4일오전 14:08분 강의참조
    validation_split=0.2, #9/4일오전 13:30분 강의참조
    subset='validation',
    seed=SEED
)
# 3분할(Train/Validation/Test)을 위해 필요한 “추가 처리”
#여기서 추가 처리란 take()와 skip()으로 val/test를 나누는 단계를 의미
#즉, validation_split만으로는 3분할을 만들 수 없고, 개발자가 val+test를 나눈다는 것
...
def build_efficientnet_model(input_shape, num_classes):
    ...
    x = Dense(512, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)  # L2 정규화
    x = Dropout(0.4)(x)  # Dropout 강화  9/3일오후 영상에서 01:07분 설명중  책431 
